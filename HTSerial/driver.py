...
from bibliopixel.util.enum import resolve_enum
from bibliopixel.drivers.return_codes import RETURN_CODES, print_error, BiblioSerialError
...
class Serial(DriverBase):
    ...
    def __init__(self, ledtype=None, num=0, dev="",
                 ...
                 baudrate=921600, connect_delay=0, **kwds):
    ...
    def _connect(self):
        try:
            ...
            try:
                ...
                log.info('Waiting for display to restart...')
                time.sleep(4)
    ...
    def _compute_packet(self):
        count = self.numLEDs // 8
        self._packet = util.generate_header(CMDTYPE.PIXEL_DATA, count)

        self._render()
        buf = []
        c_bits = [1 if c > 0 else 0 for c in self._buf[::3]]
        c_bytes = [c_bits[i:i + 8] for i in range(0, len(c_bits), 8)]
        buf = [sum([b[i] * (1 << i) for i in range(8)]) for b in c_bytes]
